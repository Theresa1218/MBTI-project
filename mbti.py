...
# ==========================================
# 3. Tools Definition (Parser & Visuals)
# ==========================================
...
# ==========================================
# 6. Phase 2: Chat Agent (FIXED JSON OUTPUT)
# ==========================================
def chat_response_generator(user_input, api_key):
    ...
    # ⚠️ FIXED PROMPT: Explicitly ask for PLAIN TEXT for advice
    system_prompt = f"""
    ...
    INSTRUCTIONS:
    1. If user asks for "chart", "graph", "visualize", "光譜":
       Return JSON: {{ "tool": "tool_generate_bipolar_chart" }}
       
    2. If user asks for "compatibility", "score" (AND there are EXACTLY 2 people):
       Return JSON: {{ "tool": "tool_calculate_compatibility" }}
       
    3. For advice, questions, or conflict resolution:
       Do NOT use JSON. Just write your advice in plain text.
    ...
    try:
        ...
        # --- FIX START: CLEAN JSON OUTPUT ---
        # If AI returns { "response": "..." }, we extract just the text.
        tool_name = None
        final_message = content # Default to raw content
        ...
        try:
            ...
            if match:
                ...
                if json_data.get("tool"): tool_name = json_data.get("tool")
                elif json_data.get("name"): tool_name = json_data.get("name")
                
                # Check if it's just a text response wrapped in JSON
                if json_data.get("response"):
                    final_message = json_data.get("response")
        except:
            # If JSON parsing fails, just treat it as plain text
            pass
        # --- FIX END ---

        # 🔧 Logic checks both the parsed JSON tool name AND raw string
        if tool_name == "tool_generate_bipolar_chart" or "tool_generate_bipolar_chart" in content:
        ...
        elif tool_name == "tool_calculate_compatibility" or "tool_calculate_compatibility" in content:
            if len(results) == 2:
                s1 = results[0]['scores']
                s2 = results[1]['scores']
            ...
            # Return the cleaned text (no JSON brackets)
...
if uploaded_file and api_key:
    ...
    if st.session_state.parsed_speakers:
        ...
        if not st.session_state.analysis_done:
            ...
            selected_names = st.multiselect("Who do you want to analyze?", speakers, default=speakers)
